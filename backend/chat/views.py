...
class PasswordRecoveryViewSet(viewsets.ViewSet):
    ...
    def check_email(self, post):
        ...
        try:
            ...
            key = int(''.join([str(random.randint(0, 9)) for i in range(4)]))
            mess = 'Ваш временный ключ для восстановления пароля ' + str(key) + '\n' + \
                   'Время жизни ключа 5 минут, если вы не успели его ввести, \n ' \
                   'то заново откройте раздел смены пароля и повторите действия. \n' \
                   '________________ \n \n' \
