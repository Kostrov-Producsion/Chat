...
class AsyncPermissionConsumer(AsyncConsumer):
    ...
    def permission(self, slug, txt_message_ban, FriendHid):
        ...
        if txt_message_ban == 'Доступно всем':
            ...
            for friend in permission.HidFriend.all():  # извлекаем всех текущий друзей
                arr.append(friend.id)
            result = list(set(listIdFriend) - set(
                arr))  # проверяем их принадлежность на дружбу и извлекаем тех которые должны быть исключены
            ...
            for friend in listIdFriend:
                permission.HidFriend.remove(friend)  # мы просто удаляем его из списка
